...
class Attention(nn.Module):
    def __init__(
        self, dim, seq_len, causal=True, heads=8, dim_head=64, dropout=0.0, stable=False
    ):
        super().__init__()
...
class SparseAxialCausalAttention(nn.Module):
    ...
    def forward(self, x, mask=None, rotary_pos_emb=None):
        ...
        split_axis_einops = (
            "b (h w) c -> b h w c" if axis == 0 else "b (h w) c -> b w h c"
        )
        merge_axis_einops = (
            "b x n d -> b (x n) d" if axis == 0 else "b x n d -> b (n x) d"
        )

        # split out axis

        q_img, k_img, v_img = map(
            lambda t: rearrange(t, split_axis_einops, h=img_size), (q_img, k_img, v_img)
        )
        ...
        bh, x, i, j = dots.shape
        causal_mask = (
            torch.ones(i, img_size, device=device).triu_(img_size - i + 1).bool()
        )
        ...
        attn_image_to_text, attn_image_to_image = (
            attn[..., :text_len],
            attn[..., text_len:],
        )
