...
class PreShiftToken(nn.Module):
    ...
    def forward(self, x, **kwargs):
        n = x.shape[1]
        seq_len, image_size, num_images = self.seq_len, self.image_size, self.num_images
...
class Transformer(nn.Module):
    def __init__(
        ...
        *,
        ...
        for ind, attn_type in zip(
            range(depth), attn_type_layer
        ):
            if attn_type == "full":
        ...
        if rotary_emb:
            assert (
                "mlp" not in attn_types
            ), "you cannot use gMLPs if rotary embedding is turned on"
